...
@Composable
fun TimerUI(viewModel: TimerViewModel) {
...
@Composable
fun StartUI(viewModel: TimerViewModel) {
...
@Composable
fun RunningUI(viewModel: TimerViewModel, primary: Color) {
...
@Composable
fun CompletedUI(viewModel: TimerViewModel) {
    val totalMillisecond: Long by viewModel.totalMillisecondLiveData.observeAsState(0L)
...
fun RunningUIPreview() {
    MyTheme {
        Surface(color = MaterialTheme.colors.background) {
            RunningUI(TimerViewModel(), MaterialTheme.colors.primary)
        }
    }
}
